...
/**
 * Array: Represents an float resizeable array.
 * Author: gomes.chri, modi.an
 */
   ...
   public:
    ...
    void expand_(size_t min_capacity) {
        ...
        for (size_t factor = 2;; factor++) {
            ...
            if (min_capacity < capacity_ * factor) {
                // allocate a larer array and copy items over
                float* new_items = new float[capacity_ * factor];
                for (size_t i = 0; i < size_; i++) {
                    new_items[i] = items_[i];
                }
                // deallocate the old array
     ...
     * @arg o  element to add
     */
    virtual void push_back(float o) {
        // if our array is full
        if (size_ == capacity_) {
            expand_(size_ + 1);
        }
     ...
     * @arg arr  array to concat
     */
    virtual void push_back_all(FloatArray* arr) {
        for (size_t i = 0; i < arr->size(); i++) {
            push_back(arr->get(i));
        }
    }
     ...
     * @arg o  the new item
     */
    virtual void add(size_t i, float o) {
        assert(i <= size_);
        if (size_ == i) {
            push_back(o);
        } else {
     ...
     * @arg a  array of items to add
     */
    virtual void add_all(size_t i, FloatArray* a) {
        assert(i <= size_);
        if (a == nullptr || a->size() == 0) {
            return;
        }
     ...
     * @return replaced element
     */
    virtual float set(size_t i, float e) {
        assert(i < size_);
        float o = items_[i];
        items_[i] = e;
        return o;
    ...
    virtual FloatArray* clone() {
        FloatArray* items_copy = new FloatArray();
        for (size_t i = 0; i < size_; i++) {
            items_copy->push_back(items_[i]);
        }
        return items_copy;
